...
def build_KNN_graph(data, N, k):
    ...
    for i in range(N):
        ...
        edges.extend([(str(i), str(kneighbor_indexs[i][j]), distance_withneighbor[i][j]) for j in
                      range(len(kneighbor_indexs[i]))])
...
def split_KNN_graph(KNN_graph, outliers, kneighbor_indexs, neighbor_indexs, thsi):
    ...
    for edge in KNN_graph.edges:
        ...
        if p1 not in outliers and p2 not in outliers:
            if (p1 in neighbor_indexs[p2] and p2 in neighbor_indexs[p1]) or (
                    len(set(kneighbor_indexs[p1]).intersection(kneighbor_indexs[p2])) >= thsi):
...
def fun_dis(c1, c2, kneighbor_indexs, data):
    ...
    for p1 in c1:
        for p2 in c2:
            if len(set(kneighbor_indexs[p1]).intersection(kneighbor_indexs[p2])) > 0 or (
                    p2 in kneighbor_indexs[p1] and p1 in kneighbor_indexs[p2]):
                ep_c1.append(p1)
                ep_c2.append(p2)
...
def merge(subgraph, K, outliers, kneighbor_indexs, data):
    ...
    while len(res_cluster) > K:
        ...
        rc_i, rc_j = (mindi_index[0][0], mindi_index[1][0]) if len(mindi_index[0]) <= 1 else sec_screen(mindi_index,
                                                                                                        res_cluster,
                                                                                                        data)
...
def assign_outliers(res_subgraph, outliers, data):
    ...
    while len(outliers) > 0:
        ...
        for i, op in enumerate(outliers):
            dm[i, :] = [np.amin(distance.cdist([np.array(data)[op]], np.array(data)[gra], metric='euclidean')) for gra
                        in res_subgraph]
...
def nthc(data, K, ka=10, la=0.2, si=0.65):
    ...
    if len(subgraph) == K:
        ...
        res_subgraph = merge(subgraph, K, outliers, kneighbor_indexs, data) if len(subgraph) > K else cut(subgraph, K,
                                                                                                          kneighbor_indexs)
